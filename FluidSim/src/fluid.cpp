...
void Fluid::step() {
    addSource(vel_x, vel_x_prev);
    addSource(vel_y, vel_y_prev);
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            ...
            switch (color) {
                ...
                case MULTI: glColor3f(d*i, d*i, d); break;
                case WHITE:
...
void Fluid::addSource(std::vector<float>& x, const std::vector<float>& s) {
    for (int i = 0; i < N*N; ++i) x[i] += dt * s[i];
}
...
void Fluid::project(std::vector<float>& u, std::vector<float>& v, std::vector<float>& p, std::vector<float>& div) {
    for (int j = 1; j < N - 1; ++j) {
        for (int i = 1; i < N - 1; ++i) {
            div[IDX(i,j)] = -0.5f * (
